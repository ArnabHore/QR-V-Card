...
class ViewDetailsViewController: UIViewController, UITableViewDataSource, UITableViewDelegate {
    ...
    override func viewDidLoad() {
        ...
        self.view.backgroundColor = UIColor.init(colorLiteralRed: 0, green: 0, blue: 0, alpha: 0.6)
    ...
    @IBAction func addToContactButtonTapped(_ sender: AnyObject) {
        if scannedData != nil {
            if addToContactDelegate != nil {
                addToContactDelegate.addToContact(shouldAdd: true, WithData: scannedData)
            }
            self.dismiss(animated: true, completion: nil)
        } else {
            print("No data")
